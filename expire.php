...
if ($expire_privat):
	...
	if ($rows>0):
		...
		while($i<$rows):
			...
			// Chat-Zeile löschen
			$result3=mysql_query("DELETE FROM chat WHERE c_id=$row->c_id",$conn);
			$i++;
		endwhile;
		mysql_free_result($result);
	endif;
...
if ($result && mysql_num_rows($result)>0):
	while($row=mysql_fetch_object($result)):
		...
		$result3=mysql_query("DELETE FROM iignore WHERE i_user_aktiv=$row->u_id OR i_user_passiv=$row->u_id",$conn);
...
if ($zeit=="03:10") {
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM invite WHERE inv_id=".$row->inv_id,$conn);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM bild WHERE b_id=".$row->b_id,$conn);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM mail WHERE m_id=".$row->m_id,$conn);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM aktion WHERE a_id=".$row->a_id,$conn);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM userinfo WHERE ui_id=".$row->ui_id,$conn);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM blacklist WHERE f_id=".$row->f_id,$conn);
	mysql_free_result($result);
	...
	if ($result)
		while ($row=mysql_fetch_object($result))
			$result3=mysql_query("DELETE FROM iignore WHERE i_id=".$row->i_id,$conn);
	mysql_free_result($result);
	...
	if ((strlen($STAT_DB_HOST) > 0))
	{
		...
		{
		    $r0 = @mysql_query("SELECT SQL_BUFFER_RESULT DISTINCT o_vhost FROM online WHERE o_raum>0",$conn);
		}
				...
				while ($i < $n)
				{
					...
					else
					{
						$o_vhost = @mysql_result($r0, $i, "o_vhost");
					        $r1 = @mysql_query("SELECT COUNT(o_id) AS anzahl FROM online WHERE o_vhost='" . AddSlashes($o_vhost) . "' ",$conn);
					}
		...
		if ((isset($onlinevhosts)) && (count($onlinevhosts) > 0))
		{
			...
			{
				$r0 = mysql_query("SELECT SQL_BUFFER_RESULT c_users FROM chat WHERE DATE_FORMAT(c_timestamp,'%Y-%m-%d %H') = '$currentdate' AND c_host='" . AddSlashes($name) . "' ORDER BY c_users DESC",$conn2);
				...
				if ($r0)
				{
					...
					if ($n == 0)
					{
							...
							/* angelegt.																							*/
						@mysql_query("INSERT INTO chat (c_timestamp, c_users, c_host) VALUES (NOW(),$nr,'" . AddSlashes($name) . "')",$conn2);
					}
					else
					{
						...
						if ($nr > $currentnr)
						{
							@mysql_query("UPDATE chat SET c_users=$nr WHERE DATE_FORMAT(c_timestamp,'%Y-%m-%d %H') = '$currentdate' AND c_host='" . AddSlashes($name) . "'",$conn2);
						}
					}
				}
			}
