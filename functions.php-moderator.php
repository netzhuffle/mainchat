...
function zeige_moderations_antworten($o_raum, $answer = "")
{
    ...
    if ($result > 0) {
        ...
        while ($row = mysql_fetch_object($result)) {
            ...
            print 
                "<A HREF=\"#\" onclick=\"opener.parent.frames['schreibe'].location='schreibe.php?http_host=$http_host&id=$id&text=";
            print urlencode($row->c_text);
            print "'; return(false);\">";
            print "$row->c_text</a></small></td><td align=right><small>";
            print 
                "<a href=moderator.php?id=$id&http_host=$http_host&mode=answeredit&answer=$row->c_id>$t[mod12]</a> ";
            print 
                "<a href=moderator.php?id=$id&http_host=$http_host&mode=answerdel&answer=$row->c_id>$t[mod13]</a> ";
            print "</small></td>";
            print "</tr>";
        }
