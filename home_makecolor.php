<?php
	if (isset($QUERY_STRING) && $QUERY_STRING=="") $QUERY_STRING="help";
	...
	if (substr($fg,0,1)=="#") $fg=substr($fg,1);
	if (substr($bg,0,1)=="#") $bg=substr($bg,1);
	...
	if ($text=="") $text="mainChat";
	if ($font=="" || $font<0 || $font>5) $font=3;
	...
	if ($yp=="")  $yp="c";
	switch($xp) {
		case "r" :  $tx=$x-$xlen; break;
		case "l" :	$tx=0; break;
		case "c" :  $tx=floor(($x-$xlen)/2); break;
		default  :	$tx=$xp;
	}
	switch($yp) {
		case "b" :  $ty=$y-$ylen; break;
		case "t" :	$ty=0; break;
		case "c" :  $ty=floor(($y-$ylen)/2); break;
		default  :	$ty=$yp;
	}
	// Set Text
